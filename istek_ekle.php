...
$pdo = getDB();
$bugun = date('Y-m-d');
$minTarih = date('Y-m-d', strtotime('-5 days'));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax'])) {
    ...
    $guid     = $_POST['guid'] ?? '';
    $usercode = $_SESSION['USERCODE'] ?? 1; // Zorunlu, yoksa 1
    ...
    if (!$aciliyet || !in_array($aciliyet, ['Düşük', 'Normal', 'Acil'])) {
        echo json_encode(['success'=>false, 'message'=>'Geçerli bir aciliyet seçmelisiniz.']);
        exit;
    }
    if (mb_strlen($aciklama) < 30) {
        echo json_encode(['success'=>false, 'message'=>'Açıklama en az 30 karakter olmalı.']);
        exit;
    }
    if (!$tarih) {
        echo json_encode(['success'=>false, 'message'=>'Tarih seçilmelidir.']);
        exit;
    }
    if (!$guid) {
        echo json_encode(['success'=>false, 'message'=>'Sistemsel bir hata oluştu. Sayfayı yenileyin.']);
        exit;
    }
    ...
    $sql = "INSERT INTO istekler_master
            (BASLIK, ISTEK, TARIH, ACILIYET, MODUL, KAYITDATE, USERCODE, GUID)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $pdo->prepare($sql);
    $ok = $stmt->execute([
        $baslik, $aciklama, $tarih,
        $aciliyet, $modul,
        $now, $usercode, $guid
    ]);
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    $guid = $pdo->query("SELECT UUID()")->fetchColumn();
...
<head>
   ...
   <meta name="description" content="Softmed İstek / Arıza Talep Ekleme Sayfası">
    <!--! The above 6 meta tags *must* come first in the head; any other head content must come *after* these tags !-->
    <!--! BEGIN: Apps Title-->
    <title>Softmed || İstek Ekle</title>
    <!--! END:  Apps Title-->
    <!--! BEGIN: Favicon-->
            ...
            <div class="navbar-content">
                <ul class="nxl-navbar">
                  ...
                  <a href="javascript:void(0);" class="nxl-link">
                        ...
                        <span class="nxl-mtext">İstekler</span><span class="nxl-arrow"><i class="feather-chevron-right"></i></span>
                 </a>
                 <ul class="nxl-submenu">
                    <li class="nxl-item"><a class="nxl-link" href="index2.php">İstekleri Gör</a></li>
                    <li class="nxl-item"><a class="nxl-link" href="istek_ekle.php">Yeni İstek Ekle</a></li>
                </ul>
                    </li>
                    <li class="nxl-item nxl-hasmenu">
                        <a href="javascript:void(0);" class="nxl-link">
...
<div class="container py-4">
  ...
  >
    <!-- GUID (gizli alan) -->
    <input type="hidden" name="guid" id="guid" value="<?= htmlspecialchars($guid) ?>">
    ...
    <div class="col-12 mb-3">
      ...
      <select id="aciliyet" name="aciliyet" class="form-control" required>
    ...
    <div class="col-12 mb-3">
      ...
      <textarea 
        ...
        class="form-control" 
        minlength="30" 
        placeholder="En az 30 karakter giriniz" 
        required
      ></textarea>
    </div>
    ...
    <div class="col-12 mb-3">
      <label for="dosyaInput" class="form-label">Dosya</label>
      <input
        type="file"
        id="dosyaInput"
        name="dosyalar[]"
      ...
      <button 
        ...
        disabled
      >Dosya Yükle</button>
      <div id="dosyaListesi" class="mt-2"></div>
    </div>

    <!-- Kaydet / Vazgeç -->
    <div class="col-12 d-flex justify-content-end gap-2">
      <a href="index2.php" class="btn btn-secondary">Vazgeç</a>
...
<script>
  ...
  const uploadBtn = document.getElementById('dosyaYukleBtn');
  ...
  async function listele() {
    const res = await fetch(`upload.php?action=list&guid=${guid}`);
    const { files } = await res.json();
    const D = document.getElementById('dosyaListesi');
    D.innerHTML = files && files.length
      ? files.map(f => `
          <div>
            <a href="${f.url}" target="_blank">${f.name}</a>
            <button data-id="${f.id}" class="silBtn btn btn-sm btn-danger ms-2">Sil</button>
          </div>
        `).join('')
      : '<small>Henüz dosya yok.</small>';
    D.querySelectorAll('.silBtn').forEach(btn => {
      btn.onclick = async () => {
        const fd2 = new FormData();
        fd2.append('action', 'delete');
        fd2.append('id', btn.dataset.id);
        await fetch('upload.php', { method: 'POST', body: fd2 });
        listele();
      };
    });
  }
  ...
  // Formu kaydet (sessiz)
  document.getElementById('istekForm').addEventListener('submit', async function(e) {
    e.preventDefault();
    const fd = new FormData(this);
    fd.append('ajax', '1');
    try {
      const res = await fetch('istek_ekle.php', { method: 'POST', body: fd });
      const j = await res.json();
      if (j.success) {
        window.location.href = 'index2.php'; // Sessiz yönlendirme
      }
    } catch (err) {
